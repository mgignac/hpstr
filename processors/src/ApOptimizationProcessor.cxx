...
void ApOptimizationProcessor::initialize(std::string inFilename, std::string outFilename) {
    ...
    if (debug_) std::cout << "[ApOptimizationProcessor]::Mass Window: " << lowMass_ << " - " << highMass_ << std::endl;
    ...
    std::cout << "[ApOptimizationProcessor]::Initializing Signal Variable Histograms" << std::endl;
    ...
    std::cout << "[ApOptimizationProcessor]::Initializing Background Variable Histograms" << std::endl;
    ...
    std::cout << "[ApOptimizationProcessor]::Initializing Test Cut Variable Histograms" << std::endl;
    ...
    std::cout << "[ApOptimizationProcessor]::Initializing Iterative Result Histograms" << std::endl;
    ...
    std::cout << "[ApOptimizationProcessor]::Initializing Test Cut Analysis Histograms" << std::endl;
    for (range_cut_iter_ it = testCutsPtr_->begin(); it != testCutsPtr_->end(); it++) {
        ...
        sig_h->SetTitle(("; " + var + "; normalized units").c_str());
        ...
        TGraphErrors* g_Nevents_ztail =
            processorHistos_->configureGraph(("g_Nevents_ztail_vs_" + name).c_str(), "", "N_{events} past z_{cut}");
        processorHistos_->addGraph(g_Nevents_ztail);
    ...
    std::cout << "[ApOptimizationProcessor]::Calculating Signal Vertex Efficiency" << std::endl;
...
bool ApOptimizationProcessor::process() {
    ...
    std::cout << "[ApOptimizationProcessor]::Filling initial signal histograms" << std::endl;
    ...
    std::cout << "[ApOptimizationProcessor]::Filling initial background histograms" << std::endl;
    ...
    for (range_cut_iter_ it = testCutsPtr_->begin(); it != testCutsPtr_->end(); it++) {
        ...
        if (cutname == "pos_z0" || cutname == "ele_z0" || cutname == "min_y0") {
            ...
            if (persistentCutsSelector_->getCutRange(cutname).first < -999.) {
                cutvalue = persistentCutsSelector_->getCutRange(cutname).second;
            } else if (persistentCutsSelector_->getCutRange(cutname).second > 999.) {
                cutvalue = persistentCutsSelector_->getCutRange(cutname).first;
            }

            TH1F* hist = (TH1F*)pdf_signal_cut->Clone(("c_pdf_signal_cut_" + cutname).c_str());
            hist->Scale(1.0 / hist->Integral());
            bool forward = testCutsSelector_->getCutRange(cutname).first < -999 ? false : true;
            double initial_cut_frac =
                hist->GetCumulative(forward)->GetBinContent(hist->GetCumulative(forward)->FindBin(cutvalue));

            // testCutHistos_->addHisto1d(hist->GetCumulative(forward));

            double* quantiles =
                getQuantileArray(testCutsSelector_->getCutRange(cutname), n_quantiles_, initial_cut_frac);
            // double* quantiles = new double[n_quantiles_]{0.3, 0.35, 0.4, 0.45};
        ...
        for (int iteration = 0; iteration < max_iteration_; iteration++) {
            ...
            //     {("h_zvtx_signal_" + cutname + "_" + std::to_string(iteration)).c_str(),
            //     ";z_{vtx}/mm;N_{events}/a.u.",
            //      200, -50.0, 150.0},
            //     "vertex_z"));
            ...
            int Nsig_for_best_zcut = 0;
            int Nbkg_for_best_zcut = 0;
            ...
            if (fixed_zcut_ < 0) {
                ...
                for (double ztail_events = start_ztail_events_; ztail_events >= min_ztail_events_;
                     ztail_events -= 0.5) {
                    double z_cut =
                        fit_params_vec.at(2) + fit_params_vec.at(3) / fit_params_vec.at(0) *
                                                   (fit_params_vec.at(0) * fit_params_vec.at(0) / 2. -
                                                    Math::log(ztail_events * fit_params_vec.at(0) /
                    ...
                    //     all_cuts_sig.Histo1D({("h_zvtx_signal_" + cutname + "_zcut_" +
                    //     std::to_string(iteration)).c_str(),
                    //                           ";z_{vtx}/mm;N_{events}/a.u.", 200, -50.0, 150.0},
                    //                          "vertex_z"));
                    ...
                    if (debug_) {
                        std::cout << "Test Cut " << cutname << " at iteration " << iteration << " (cutting "
                                  << cutSignal * 100 << "% of signal in this variable). bkg: " << Nbkg
                                  << " sig: " << Nsig << std::endl;
                    }
            ...
            if (ZBi_for_best_zcut > best_ZBi) {
                best_ZBi = ZBi_for_best_zcut;
                best_cutval = cutvalue;

                std::cout << "z cut for " << cutname << " " << best_z_cut << " nevents " << Nevents_for_best_zcut
                          << std::endl;
            }
            ...
            if (iteration > 0) {
                if (cutname == "pos_z0" || cutname == "ele_z0" || cutname == "min_y0") {
                    ...
                    processorHistos_->getGraph("g_Nevents_ztail_vs_" + cutname)
                        ->AddPoint(iteration, Nevents_for_best_zcut);
                } else {
                    ...
                    processorHistos_->getGraph("g_Nevents_ztail_vs_" + cutname)
                        ->AddPoint(cutvalue, Nevents_for_best_zcut);
                }
            }
        }
    ...
    // save sig_hists, bkg_hists to outfile_, change to "UPDATE" mode if snapshot above
...
std::vector<double> ApOptimizationProcessor::fitZBkgTail(RDF::RResultPtr<TH1D> h_bkg, std::string fitname,
    ...
    if (!h_bkg || h_bkg->GetEntries() == 0) {
        std::cout << "Error: h_bkg is null or empty!" << std::endl;
        return fit_params;
    }
...
std::vector<std::pair<double, double>> ApOptimizationProcessor::getZoffsetAlpha(RDF::RResultPtr<TH2D> h_y0_vs_z,
                                                                                int n_quantiles, int nbins) {
